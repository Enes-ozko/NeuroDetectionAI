...
def lancer_batch():
    ...
    total = len(fichiers)
    if total == 0:
        print("Aucun fichier _t1.jpg trouve !")
        return

    print(f"Demarrage du traitement par lot : {total} coupes trouvees.\n")
    ...
    print("\n" + "="*60)
    print(f"TRAITEMENT TERMINE ! Dossier : {DIR_OUTPUT}")
    print("="*60)

    if len(alertes_tumeur) > 0:
        print(f"\nALERTE : {len(alertes_tumeur)} coupes presentent une probabilite de tumeur > 70% :\n")
        alertes_tumeur.sort(key=lambda x: x[1], reverse=True)
        for fichier, score in alertes_tumeur:
            print(f"  {score * 100:>5.1f}% | {fichier}")
    else:
        print("\nBilan : Aucune coupe n'a depasse le seuil d'alerte de 70%.")
